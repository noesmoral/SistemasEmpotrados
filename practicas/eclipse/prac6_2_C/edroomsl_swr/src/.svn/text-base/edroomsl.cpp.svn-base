...
void CEDROOMMessage::FreeDeliveryInfo(){;} //Nothing for regular Msgs
	...
	if(m_IsSenderWaiting)
	{
		...
		if(pDataPool){
			if(pDataPool->IsEmpty())
				mp_Interface->mp_Component->SetErrorFlag(EDROOMReplyDataPoolEmpty);
		}
...
void CEDROOMTimeOutMessagesPool::SetMemory(TEDROOMUInt32 elemNumber
	...
	CEDROOMMemoryPool::SetMemory(elemNumber,pDataPool
			, pPoolMarks, sizeof (CEDROOMTimeOutMessage));
...
CEDROOMNonMaskIRQInterface::CEDROOMNonMaskIRQInterface(
																	Pr_IRQHandler irqHandler
  																,Pr_IRQHandler idleIrqHandler
																	,Pr_IRQEvent &IRQEventLook
																	,Pr_SemaphoreBin & endIrqLook
                                  ,TEDROOMUInt8 irqVectorCount
                                  ,TEDROOMUInt8 *pirqVectors):CEDROOMIRQInterface(
                                  														irqHandler
                                                              , idleIrqHandler
                                                              , IRQEventLook
                                                              , endIrqLook, 0){
...
// Funcin Amiga funEDROOMTimer que implementa el timer
	...
	//CEDROOMTimerInfo TimerInfo;    //Timer Information
		...
		while(receiveTimerInfo && ptimerSAP->mp_NextTimer){
				...
				if (timerConfig.m_Time > currentTime){
					...
					if(pComponent->m_MsgQueue.IsFull()){
						...
						pComponent->m_MsgQueue.In(
											timerConfig.mp_TimerInfo->mp_Msg);
...
CEDROOMTimer       CEDROOMTimingSAP::InformAt(CEDROOMTimingInterface* interf
		...
		if(		mp_TimeOutMsgPool->IsEmpty()
				||mp_TimerInfoPool->IsEmpty()){
...
CEDROOMQueue::CEDROOMQueue(TEDROOMUInt32  Max_Num_Mess,
					Pr_Task * pTask,
...
void CEDROOMQueue::CEDROOMQueueNodesPool::SetMemory(TEDROOMUInt32 elemNumber
								, CQueueNode * pPoolData
								,TEDROOMBool * pPoolMarks){

   CEDROOMMemoryPool::SetMemory(elemNumber,pPoolData
		   , pPoolMarks, sizeof (CEDROOMQueue::CQueueNode));
...
void CEDROOMLocalTXChannel::send (TEDROOMSignal signal
									, void *p_data
...
CEDROOMComponent::CEDROOMComponent( TEDROOMComponentID componentID
							...
							, TEDROOMPriority prioTask,TEDROOMUInt32 stack
							,CEDROOMComponentMemory * pComponentMemory	 ):
										 EDROOMName("Cmp")										 ,m_ComponentID(componentID)
										 ,m_SynchrMsgPool(&m_SynchrMsgOut)
										 ,m_ComponentTask(EDROOMComponentTask
												 ,prioTask,stack)
										 ,m_MsgQueue(numMaxM
